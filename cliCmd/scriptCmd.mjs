    ...
    .action(async (options) => {
        ...
        for ( let row of sheetData ) {
            ...
            if ( isNotEmpty(row["SSH User"]) && isNotEmpty(row["SSH Host"]) ) {
                ...
                try {
                    //let call = `curl "https://binaries.Boss-net.com/connector/setup.sh" | sudo BOSSNET_ACCESS_TOKEN="${row["Access Token"]}" BOSSNET_REFRESH_TOKEN="${row["Refresh Token"]}" BOSSNET_LOG_ANALYTICS="v1" BOSSNET_URL="https://${networkName}.Boss-net.com" bash`
