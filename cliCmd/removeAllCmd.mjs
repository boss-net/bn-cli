...
export const removeAllCmd = new Command()
    ...
    .action(async (options) => {
        ...
        if ( options.groups ) {
            let allGroups = await client.fetchAllGroups({fieldSet: [BossnetApiClient.FieldSet.ID], fieldOpts:{extraFields: ["type"]}});
        ...
        if ( options.resources ) {
            let allResources = await client.fetchAllResources({fieldSet: [BossnetApiClient.FieldSet.ID]});
            await Promise.all(allResources.map(resource => client.removeResource(resource.id)));
        }

        if ( options.remoteNetworks ) {
            let allRemoteNetworks = await client.fetchAllRemoteNetworks({fieldSet: [BossnetApiClient.FieldSet.ID]});
