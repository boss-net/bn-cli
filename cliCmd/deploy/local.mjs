...
export const deployLocalVmCommand = new Command()
    .description("Deploy Boss-net Connector as a local Virtual Machine (requires Multipass - visit https://multipass.run)")
    .action(async (options) => await (new LocalVmDeployer(options)).deploy());

export const deployLocalContainerCommand = new Command()
    .description("Deploy Boss-net Connector as a Docker Container (requires Docker - visit https://www.docker.com/)")
...
export const deployCloudInitCommand = new Command()
    .description("Output a Cloud-Init file for bootstrapping a Boss-net connector - visit https://cloudinit.readthedocs.io/ for more information")
    .action(async (options) => await (new CloudInitDeployer(options)).deploy());

export const deployLocalCommand = new Command()
    .description("Deploy Boss-net connector locally/on-prem.")
